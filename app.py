...
def render_objetivos(cliente="", modo="cliente"):
    ...
    max_columnas_tablero = 3

    for inicio_grupo in range(
        0,
        len(grupos_tablero),
        max_columnas_tablero,
    ):
        grupos_fila = grupos_tablero[
            inicio_grupo:
            inicio_grupo + max_columnas_tablero
        ]
        columnas_tablero = st.columns(
            len(grupos_fila)
        )

        for indice, grupo_tablero in enumerate(
            grupos_fila
        ):
            with columnas_tablero[indice]:
...
def siguiente_fecha_tarea(fecha_actual, frecuencia, intervalo=1):
    ...
    if frecuencia == "Diaria":
        ...
        siguiente = fecha + pd.DateOffset(months=6 * intervalo)
    ...
    elif frecuencia == "Último día hábil del mes":
        mes_objetivo = fecha + pd.DateOffset(months=intervalo)
        siguiente = (
            pd.Timestamp(
                year=mes_objetivo.year,
...
def render_tareas_internas(cliente_fijo="", modo="admin"):
    ...
    if estados_filtro:
        mascara_estado = pd.Series(False, index=tareas_vista.index)
        if "Activas" in estados_filtro:
            mascara_estado |= estado_serie.ne("Finalizada")

        estados_concretos = [
            valor for valor in estados_filtro if valor != "Activas"
        ]
        if estados_concretos:
            mascara_estado |= estado_serie.isin(estados_concretos)

        tareas_vista = tareas_vista[mascara_estado].copy()
    else:
        # Las finalizadas sólo se muestran con selección explícita.
        tareas_vista = tareas_vista[estado_serie.ne("Finalizada")].copy()
